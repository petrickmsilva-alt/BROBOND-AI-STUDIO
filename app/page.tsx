...
import { API_URL, Asset, AuthUser, Job, LoraVersion, authenticate, cancelJob, createImageJob, createPersona, createVideoJob, enhancePrompt, expandStoryboard, listAssets, listPersonaLoras, trainPersona, uploadAsset } from '../lib/api';
import {
...
function ImageStudio({ prompt, setPrompt, generated, setGenerated }: { prompt: string; setPrompt: (s: string) => void; generated: boolean; setGenerated: (b: boolean) => void }) {
  ...
  const handleReference = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.remote) { setReferenceAssetId(result.data.id); setReferenceStatus('Reference ready'); } else setReferenceStatus('API offline');
    event.target.value = '';
  };
  useEffect(() => { const personaId = typeof window !== 'undefined' ? localStorage.getItem('brobond_persona_id') : null; if (personaId) listPersonaLoras(personaId).then(result => { if (result.remote) setLoras(result.data); }); }, []);
  ...
  const generate = async () => {
    setLoading(true);
    const result = await createImageJob({ prompt, model: 'flux-1.1-pro-ultra', aspect_ratio: '16:9', resolution: '2048', guidance_scale: 7.5, steps: 28, lora_id: selectedLora || undefined, controlnet, controlnet_scale: controlScale, ip_adapter_scale: ipScale, reference_asset_id: referenceAssetId || undefined });
  ...
  return <StudioLayout type="Image" onGenerate={generate}><div className="studio-grid"><div className="control-panel"><div className="panel-heading"><span>Prompt</span><button className="magic-button" onClick={enhance}><WandSparkles size={14} /> {enhancing ? 'Enhancing...' : 'Enhance'}</button></div><textarea value={prompt} onChange={e => setPrompt(e.target.value)} /><div className="prompt-meta"><span>72 / 2,000</span><button>Reset</button></div><label>Negative prompt <span>Optional</span></label><input placeholder="Things to avoid in your image..." /><div className="form-row"><label>Model<select><option>Flux 1.1 Pro Ultra</option><option>Flux Dev</option></select></label><label>Aspect ratio<select><option>16:9 · Landscape</option><option>1:1 · Square</option><option>9:16 · Portrait</option></select></label></div><div className="form-row"><label>Resolution<select><option>2048 × 1152 · 2K</option><option>1024 × 576 · HD</option><option>4096 × 2304 · 4K</option></select></label><label>Seed<div className="input-with-action"><input value="Random" readOnly /><button><Aperture size={14} /></button></div></label></div><div className="conditioning-box"><div className="panel-heading"><span>Structure & identity</span><span className="muted">Optional</span></div><label>ControlNet<select value={controlnet} onChange={event => setControlnet(event.target.value)}><option value="none">None</option><option value="pose">OpenPose</option><option value="depth">Depth map</option><option value="canny">Canny edges</option><option value="tile">Tile detail</option></select></label><div className="mini-slider"><span>Control strength <b>{controlScale.toFixed(1)}</b></span><input type="range" min="0" max="2" step="0.1" value={controlScale} onChange={event => setControlScale(Number(event.target.value))} /></div><div className="mini-slider"><span>IP Adapter <b>{ipScale.toFixed(1)}</b></span><input type="range" min="0" max="1" step="0.1" value={ipScale} onChange={event => setIpScale(Number(event.target.value))} /></div></div><div className="slider-row"><span>Guidance scale <b>7.5</b></span><input type="range" defaultValue="54" /></div><div className="slider-row"><span>Steps <b>28</b></span><input type="range" defaultValue="43" /></div><div className="panel-footer"><label className="secondary-button lora-select"><Plus size={15} /> {loras.length ? <select value={selectedLora} onChange={event => setSelectedLora(event.target.value)}><option value="">Add LoRA</option>{loras.map(lora => <option value={lora.asset_id} key={lora.asset_id}>{lora.version}</option>)}</select> : 'Add LoRA'}</label><label className="secondary-button upload-label"><ImageIcon size={15} /> {referenceStatus || 'Reference image'}<input type="file" accept="image/*" onChange={handleReference} /></label></div></div><div className={`generation-canvas ${generated ? 'has-result' : ''}`}>{generated ? <><div className="result-art"><div className="result-light" /><div className="result-person"><div className="head" /><div className="body" /></div><span className="result-label">FLUX / 2K</span></div><div className="result-toolbar"><span>{loading ? 'Submitting generation...' : `${connection} · ready`}{loading && <i className="job-progress"><b style={{ width: `${progress}%` }} /></i>}</span><div>{job && (job.status === 'queued' || job.status === 'running') && <button className="cancel-job" onClick={async () => { await cancelJob(job.id); setConnection('Job cancelled'); setLoading(false); }}>Cancel</button>}<button className="icon-button"><Download size={16} /></button><button className="icon-button"><MoreHorizontal size={16} /></button></div></div></> : <div className="empty-canvas"><div className="empty-icon"><Sparkles size={23} /></div><h3>Your canvas is empty</h3><p>Describe an image and hit Generate<br />to bring your idea to life.</p><span>⌘ Enter to generate</span></div>}</div></div></StudioLayout>; }
...
function VideoStudio() {
  ...
  useEffect(() => {
    if (!job?.id) return;
    const socket = new WebSocket(`${API_URL.replace(/^http/, 'ws')}/api/v1/queue/events/${job.id}`);
    socket.onmessage = event => {
      const next = JSON.parse(event.data) as Job;
  ...
  const generate = async () => {
    ...
    const result = await createVideoJob({ prompt: 'A cinematic slow dolly-in through a futuristic city at night, neon reflections on wet pavement', mode: 'text-to-video', duration_seconds: 5, fps: 24, aspect_ratio: '16:9', camera_motion: 'dolly-in', cinematic_mode: true, native_audio: true, lora_id: selectedLora || undefined });
    if (result.remote) { setJob(result.data); setProgress(result.data.progress); setConnection(`Job ${result.data.id.slice(0, 8)} queued`); }
    else { setProgress(100); setConnection('Local preview · ready'); }
  };
  return <StudioLayout type="Video" onGenerate={generate}><div className="video-layout"><div className="control-panel"><div className="tab-row"><button className="active">Text to video</button><button>Image to video</button></div><div className="panel-heading"><span>Describe your shot</span><button className="magic-button"><WandSparkles size={14} /> Enhance</button></div><textarea placeholder="A slow dolly-in through a futuristic city..." defaultValue="A cinematic slow dolly-in through a futuristic city at night, neon reflections on wet pavement" /><label>Duration <div className="chip-row"><Chip active>5s</Chip><Chip>10s</Chip><Chip>15s</Chip></div></label><label>Format <div className="chip-row"><Chip active>16:9</Chip><Chip>9:16</Chip><Chip>1:1</Chip></div></label><label>Camera motion<select><option>Dolly in</option><option>Orbit left</option><option>Static</option></select></label>{loras.length > 0 && <label>Persona LoRA<select value={selectedLora} onChange={event => setSelectedLora(event.target.value)}><option value="">None</option>{loras.map(lora => <option value={lora.asset_id} key={lora.asset_id}>{lora.version}</option>)}</select></label>}<div className="setting-line"><span>Native audio</span><Toggle /></div><div className="setting-line"><span>Cinematic mode</span><Toggle /></div></div><div className="video-canvas"><div className="video-placeholder"><div className="video-lines" /><Play size={28} fill="currentColor" /><span>{connection}</span>{job && (job.status === 'queued' || job.status === 'running') && <><div className="render-progress"><i style={{ width: `${progress}%` }} /></div><button className="cancel-job" onClick={async () => { await cancelJob(job.id); setConnection('Render cancelled'); }}>Cancel render</button></>}</div><div className="timeline"><span>00:00</span><div className="timeline-track"><i style={{ width: `${Math.max(progress, 2)}%` }} /></div><span>00:05</span></div></div></div></StudioLayout>; }

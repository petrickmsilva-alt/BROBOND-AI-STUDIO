...
│   └── tests/            # pytest suite (2,215 tests)
...
The architecture is frozen, and the freeze is enforced rather than described:

- **`docs/ARCHITECTURE_MANIFEST.md`** — the ten modules, each with an owner,
  a single responsibility, a complete allow-list of dependencies and the
  couplings it refuses. `backend/tests/test_architecture_boundaries.py` builds
  the real import graph and fails on any edge the manifest does not declare —
  including indirect ones, so a pass-through module cannot launder a forbidden
  import.
- **`docs/API_SNAPSHOT.json`** — the public surface (111 routes, 3 WebSockets,
  137 models with per-field types, auth per route), generated by
  `scripts/gen_api_snapshot.py`. Changing it is fine; changing it by accident
  is not.
- **`docs/EVENT_CATALOG.md`** — every event the platform emits, plus the four
  names reserved for the AI Core that nothing is allowed to emit yet.
- **`backend/app/contracts/`** — the shared vocabulary, defined exactly once.
  `app/core/contracts.py` stays as a re-export façade so no existing import
  breaks.

Three required CI jobs hold the line: **Architecture Guard**, **Import Boundary
Guard** and **API Snapshot Guard**.
...
The backend suite is **2,215 tests** and total backend coverage is **97%**, held by a `--fail-under=95` gate in CI. The whole

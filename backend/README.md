...
Persona training is validated through `POST /api/v1/personas/{persona_id}/train` and requires 20–50 reference asset IDs. The Celery `train_lora` task prepares `metadata.jsonl`, validates image files, and calls the configured trainer without ever fabricating a model file. For a GPU worker, install `requirements-training-gpu.txt`, set `BROBOND_TRAINING_ENABLED=true`, `BROBOND_QUEUE_ENABLED=true`, and configure `BROBOND_LORA_TRAINER_COMMAND`.

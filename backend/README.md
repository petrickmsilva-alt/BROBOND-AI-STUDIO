...
Conditioning options are exposed at `GET /api/v1/models/conditioning`. Image jobs accept `controlnet`, `controlnet_scale`, `ip_adapter_scale`, and `reference_asset_id`; the worker validates workspace ownership before passing local references to a compatible provider pipeline. Reference preprocessing is available at `POST /api/v1/assets/{asset_id}/conditioning` for `edges`, `depth`, and `tile`; `pose` is reserved for the GPU OpenPose worker.

...
def test_the_authorisation_count_is_the_real_one(inventory) -> None:
    """49 of 86 routes touch identity — 46 require it, 3 do not (V3.1).
    ...
    like personas): 49 of 86 (46 required, 3 optional). Before PR002 the
    numbers were 10 (6 required, 4 optional). The prose and the guard move
    together, because the three numbers drift independently of each other.
    The three WebSockets are also authenticated, through the `token` query
    parameter (see test_security_authorization.py) — they are not routes, so
    ...
    assert required == 46, f"rotas exigindo token mudaram: {required}"
